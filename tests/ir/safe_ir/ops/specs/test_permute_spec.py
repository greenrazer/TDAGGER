...
class TestPermuteSpec(unittest.TestCase):
    def test_basic(self):
        # doesn't raise
        # nhwc -> nchw
        PermuteSpec(permutation={1: 2, 2: 3, 3: 1})

    def test_raises_on_incomplete_cycle(self):
        with self.assertRaises(Exception):
            # nhwc -> nchw
            PermuteSpec(permutation={1: 2, 2: 3, 3: 0})

    def test_raises_on_overlapping_values(self):
        with self.assertRaises(Exception):
            # nhwc -> nchw
            PermuteSpec(permutation={1: 2, 2: 3, 3: 2})


class TestPermuteOutputSpec(unittest.TestCase):
    def test_basic(self):
        # nhwc -> nchw
        spec = PermuteSpec(permutation={1: 2, 2: 3, 3: 1})
    ...
    def test_negitive_dimensions(self):
        spec = PermuteSpec(permutation={1: -2, -2: -1, -1: 1})
    ...
    def test_raises_on_negitive_overlap(self):
        spec = PermuteSpec(permutation={2: -2, -2: -1, -1: 2})
    ...
    def test_raises_on_insufficient_shape(self):
        ...
        input_specs = [TensorSpec(shape=(1, 2, 3), data_type=DataType.FLOAT32)]
